...
const CONTRACTS = {
  blatnoy: "0xf95F4706112cE0BD4e2dEE7f024df2D9f54cd7AC",
  muzhik:  "0xf95F4706112cE0BD4e2dEE7f024df2D9f54cd7AC",
  kozyol:  "0xf95F4706112cE0BD4e2dEE7f024df2D9f54cd7AC",
  petuh:   "0xf95F4706112cE0BD4e2dEE7f024df2D9f54cd7AC",
};

// ---------- 2. Правильный ABI (на nft2me сейчас mint(to, name)) ----------
const MINT_ABI = [
  {
    inputs: [
...
const mintNFT = async () => {
  ...
  if (!ethers.isAddress(contractAddress)) {
    alert('Ошибка: неверный адрес контракта!');
    setMinting(false);
    return;
  }
  ...
  try {
    ...
    const tx = await contract.mint(account, name || 'Братан', {
      gasLimit: 300000,
      value: ethers.parseEther("0.000002") // ← ПОСТАВЬ СВОЮ ЦЕНУ!
    });

    setTxHash(tx.hash);
    alert(`ГОТОВО, ${name || 'братан'}! Ты — ${result.title}\nTX: ${tx.hash}`);
    await tx.wait();
  } catch (e: any) {
    console.error(e);
    const msg = e.reason || e.message || 'Ошибка';
    alert(`Минт не прошёл: ${msg}`);
  } finally {
    setMinting(false);
  }
};
